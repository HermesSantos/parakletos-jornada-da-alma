...
const HeroSection = ({ content = defaultLandingContent.hero }: HeroSectionProps) => {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-6 text-center">
        ...
        <h1 className="animate-fade-up-delay-1 font-serif text-5xl md:text-7xl lg:text-8xl font-light leading-tight text-cream mb-8 max-w-4xl mx-auto">
          {content.headline.map((line, index) => (
            <span key={index} className="block">
              {line.text}
              {line.highlight && (
                <span className="text-gradient-gold font-medium italic">{line.highlight}</span>
              )}
              .
            </span>
          ))}
        ...
        <p className="animate-fade-up-delay-2 text-cream/90 text-lg md:text-xl font-sans font-light max-w-3xl mx-auto mb-10 leading-relaxed text-center">
          <span className="block mb-3">{content.subtext}</span>
          <span className="block text-base md:text-lg tracking-wide opacity-90">{content.bullets}</span>
        </p>

        <div className="animate-fade-up-delay-3 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-4">
          {content.ctas
            .filter((cta) => cta.variant !== "link")
            .map((cta) => (
              <a
                key={cta.label}
                href={cta.href}
                className={
                  cta.variant === "primary"
                    ? "inline-flex items-center gap-2 rounded-full bg-primary px-8 py-3.5 text-sm font-semibold text-primary-foreground tracking-wider transition-all hover:opacity-90 hover:shadow-lg w-full sm:w-auto justify-center"
                    : "inline-flex items-center gap-2 rounded-full border border-gold/50 bg-gold/15 px-8 py-3.5 text-sm font-semibold text-cream tracking-wider transition-all hover:bg-gold/25 hover:border-gold/70 backdrop-blur-sm w-full sm:w-auto justify-center"
                }
              >
                {cta.label}
              </a>
        ...
        {content.ctas
          .filter((cta) => cta.variant === "link")
          .map((cta) => (
            <div key={cta.label} className="animate-fade-up-delay-3">
              <a
                href={cta.href}
